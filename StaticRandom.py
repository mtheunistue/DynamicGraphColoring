...
class StaticRandomAlgo:
    def __init__(self, G: nx.Graph = nx.Graph(), p=0.5, maxDegreeBound: int = None):
        ...
        if (p != 0):
            self.pinv = int(1/p)                        # Inverse of P, number of iterations until taking a random step
    ...
    def addEdge(self, s, t):
        ...
        if rb:
            ...
            coloring = nx.coloring.greedy_color(self.G)
            for node in list(self.G.nodes()):
                self.G.nodes[node]['color'] = coloring.get(node)
